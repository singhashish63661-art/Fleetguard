...
export default function ClientDashboard() {
  ...
  /** Skip duplicate toast when realtime INSERT already notified before fetchData updates counts */
  const skipIncidentCountToastRef = useRef(false)
  const skipTamperingCountToastRef = useRef(false)
  ...
  const rowMatchesClientCompany = useCallback(
    (value: string | null | undefined) => {
      const profileCo = normKey(currentUser?.company_name)
      if (!profileCo) return true
      return normKey(value) === profileCo
    },
    [currentUser?.company_name]
  )

  const triggerToast = useCallback(
    (message: string, tone: 'info' | 'success' | 'warning' = 'info', bumpUnreadBy = 1) => {
      setToast({ message, tone })
      setNotifications(prev => [{ id: `${Date.now()}`, message, tone, ts: Date.now() }, ...prev].slice(0, 10))
      if (bumpUnreadBy > 0) setUnreadCount(prev => prev + bumpUnreadBy)
  ...
  const fetchData = useCallback(async (companyFilter?: string | null) => {
    ...
    const base = supabase.from('accidents').select('*').order('created_at', { ascending: false })
    const { data: accidents } = cf ? await base.ilike('company_name', cf) : await base
    if (accidents && accidents.length === 0 && cf) {
  ...
  const fetchTamperingLogs = useCallback(async (companyFilter?: string | null) => {
    const cf = companyFilter?.trim()
    const base = supabase.from('tampering_incidents').select('*').order('created_at', { ascending: false })
    const { data } = cf ? await base.ilike('client_name', cf) : await base
  ...
  useEffect(() => {
    let cancelled = false
    ;(async () => {
      const auth = await ensureDashboardAuth(supabase, (href) => router.replace(href), 'client')
      if (cancelled) return
      if (!auth.ok) return
      setCurrentUser({ ...auth.profile, email: auth.user.email })
  ...
  useEffect(() => {
    if (!dashboardReady || !currentUser) return
    const company = currentUser?.company_name || null
    fetchData(company)
    fetchTamperingLogs(company)
  }, [dashboardReady, currentUser, fetchData, fetchTamperingLogs])
  ...
  useEffect(() => {
    ...
    if (skipIncidentCountToastRef.current) {
      skipIncidentCountToastRef.current = false
      incidentCountRef.current = data.length
      return
    }
    if (data.length > incidentCountRef.current) {
      const diff = data.length - incidentCountRef.current
      triggerToast(`New incident records: +${diff}`, 'info', diff)
    }
    incidentCountRef.current = data.length
  }, [data, triggerToast])
  ...
  useEffect(() => {
    ...
    if (skipTamperingCountToastRef.current) {
      skipTamperingCountToastRef.current = false
      tamperingCountRef.current = tamperingLogs.length
      return
    }
    if (tamperingLogs.length > tamperingCountRef.current) {
      const diff = tamperingLogs.length - tamperingCountRef.current
      triggerToast(`New tampering requests: +${diff}`, 'info', diff)
    }
    tamperingCountRef.current = tamperingLogs.length
  }, [tamperingLogs, triggerToast])

  useEffect(() => {
    if (!dashboardReady || currentUser === null) return

    let incidentChannel: ReturnType<typeof supabase.channel> | null = null
    let tamperingChannel: ReturnType<typeof supabase.channel> | null = null
    let cancelled = false

    const company = currentUser?.company_name || null

    const formatRealtimeErr = (e: unknown) => {
      if (e == null) return ''
      if (typeof e === 'object' && 'message' in e && typeof (e as { message?: string }).message === 'string') {
    ...
    const realtimeTroubleshoot =
      'Realtime: add tables to publication (run supabase/enable_realtime_notifications.sql), enable Realtime for the project in Dashboard → Database → Publications, and ensure RLS allows SELECT on rows you expect to receive.'
    ...
    void (async () => {
      ...
      // One postgres_changes binding per channel avoids "binding mismatch" CHANNEL_ERROR
      // when stacking multiple .on(postgres_changes) listeners (see supabase-js #1917).
      incidentChannel = supabase
        .channel(`client-accidents-${currentUser.id}`)
        .on(
          'postgres_changes',
          { event: '*', schema: 'public', table: 'accidents' },
          (payload) => {
            if (payload.eventType === 'DELETE') return
            const row = payload.new as { company_name?: string; vehicle_number?: string }
            if (!rowMatchesClientCompany(row.company_name)) return
            if (payload.eventType === 'INSERT') {
              skipIncidentCountToastRef.current = true
              triggerToast(`New incident logged: ${row.vehicle_number || 'new record'}`, 'info', 1)
              void fetchData(company)
            } else if (payload.eventType === 'UPDATE') {
              triggerToast(`Incident updated: ${row.vehicle_number || 'record'}`, 'success', 1)
              void fetchData(company)
            }
          }
        )
      ...
      tamperingChannel = supabase
        .channel(`client-tampering-${currentUser.id}`)
        .on(
          'postgres_changes',
          { event: '*', schema: 'public', table: 'tampering_incidents' },
          (payload) => {
            if (payload.eventType === 'DELETE') return
            const row = payload.new as { client_name?: string; vehicle_number?: string; status?: string }
            if (!rowMatchesClientCompany(row.client_name)) return
            if (payload.eventType === 'INSERT') {
              skipTamperingCountToastRef.current = true
              triggerToast(`New tampering request: ${row.vehicle_number || 'new record'}`, 'info', 1)
              void fetchTamperingLogs(company)
            } else if (payload.eventType === 'UPDATE') {
              const tone = row.status === 'Rejected' ? 'warning' : 'success'
              triggerToast(`Tampering ${row.status}: ${row.vehicle_number || 'record'}`, tone, 1)
              void fetchTamperingLogs(company)
            }
          }
        )
    ...
    return () => {
      ...
      if (tamperingChannel) void supabase.removeChannel(tamperingChannel)
    }
  }, [dashboardReady, currentUser, fetchData, fetchTamperingLogs, rowMatchesClientCompany, triggerToast])
  ...
  /** Static “needs attention” items so the bell panel is useful even before any realtime toast */
  const digestAlerts = useMemo(() => {
    const items: { id: string; message: string; tone: 'info' | 'warning' | 'success' }[] = []
    const missingVideo = data.filter((d) => !d.video_provided).length
    if (missingVideo > 0) {
      items.push({
        id: 'digest-missing-video',
        tone: 'warning',
        message: `${missingVideo} incident(s) are missing video evidence — review Master Database or Claims Pipeline.`,
      })
    }
    const pendingTampering = tamperingLogs.filter((l) => {
      const s = (l.status || '').toLowerCase()
      return s.includes('pending') || s.includes('approval')
    }).length
    if (pendingTampering > 0) {
      items.push({
        id: 'digest-tampering-pending',
        tone: 'warning',
        message: `${pendingTampering} tampering request(s) need your approval — open Tampering Devices.`,
      })
    }
    return items
  }, [data, tamperingLogs])

  const bellBadgeCount = Math.min(99, unreadCount + (digestAlerts.length > 0 ? 1 : 0))
        ...
        <div className="fixed top-20 right-6 w-80 bg-white rounded-2xl shadow-2xl border border-slate-200 z-40 overflow-hidden">
          ...
          <div className="max-h-72 overflow-y-auto divide-y divide-slate-100">
            {digestAlerts.length > 0 && (
              <div className="p-4 bg-amber-50/90 border-b border-amber-100">
                <p className="text-[10px] font-black uppercase tracking-widest text-amber-900 mb-2">Needs attention</p>
                <ul className="space-y-2">
                  {digestAlerts.map((d) => (
                    <li key={d.id} className="flex gap-2 text-sm text-slate-800">
                      <span className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${d.tone === 'warning' ? 'bg-amber-500' : 'bg-indigo-500'}`} />
                      <span className="font-medium leading-snug">{d.message}</span>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            {notifications.length === 0 && digestAlerts.length === 0 && (
              <div className="p-4 text-sm text-slate-500">No alerts yet. New incidents and tampering updates appear here when they arrive.</div>
            )}
        ...
        <header className="min-h-[5.5rem] bg-linear-to-r from-white/95 via-indigo-50/50 to-white/95 backdrop-blur border-b border-slate-300/70 flex flex-wrap items-center justify-between px-4 sm:px-6 lg:px-8 py-4 shrink-0 z-10 shadow-md shadow-slate-300/30 gap-y-4 transition-colors sticky top-0">
          ...
          <div className="flex flex-wrap items-center justify-end gap-3 flex-1">
            ...
            <button onClick={() => { setShowNotifCenter(!showNotifCenter); setUnreadCount(0) }} className="relative h-10 w-10 rounded-full bg-slate-50 border border-slate-200 flex items-center justify-center hover:bg-slate-100 transition shadow-sm shrink-0">
              ...
              {bellBadgeCount > 0 && (
                <span className="absolute -top-1 -right-1 min-w-[1rem] h-4 px-0.5 rounded-full bg-rose-500 text-white text-[10px] font-black flex items-center justify-center ring-2 ring-white">
                  {bellBadgeCount}
                </span>
              )}
            </button>
          ...
          <div id="print-area" className="bg-white rounded-2xl w-full max-w-6xl max-h-[90vh] flex flex-col shadow-2xl ring-1 ring-white/20 overflow-hidden print:max-h-none print:overflow-visible print:border-none print:shadow-none print:ring-0">
            ...
            <div className="p-8 overflow-y-auto bg-slate-50/50 print:bg-white grid grid-cols-1 lg:grid-cols-2 gap-8 print-grid print:overflow-visible flex-1">
              ...
              <div className="lg:col-span-2 print-col-span-2 mt-8 pt-6 border-t border-slate-200 text-center page-break-avoid hidden print:block">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-loose">© {new Date().getFullYear()} All Rights Reserved For Pawan&Team<br/>Confidential & Proprietary Tampering Device Data</p>
              </div>
            </div>
          </div>
        </div>
