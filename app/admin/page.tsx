...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    let cancelled = false
    ;(async () => {
      const auth = await ensureDashboardAuth(supabase, (href) => router.replace(href), 'admin')
      if (cancelled) return
      if (!auth.ok) return
      setCurrentUser({ ...auth.profile, email: auth.user.email } as Profile)
      await Promise.all([
        fetchLogs(),
        fetchTamperingLogs(),
        fetchProfiles(),
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- bootstrap runs once; fetch helpers close over latest supabase/setters
  }, [])
  ...
  const downloadExcelTemplate = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Template"); XLSX.writeFile(wb, "Shoora_Monitoring_Import_Template.xlsx");
  }
  ...
  const filteredLogs = allLogs.filter(log => {
    ...
    if (!term) return true
    const matches =
      (log.vehicle_number && String(log.vehicle_number).toLowerCase().includes(term)) ||
      (log.driver_name && String(log.driver_name).toLowerCase().includes(term)) ||
      (log.company_name && String(log.company_name).toLowerCase().includes(term)) ||
      (log.client_id_number && String(log.client_id_number).toLowerCase().includes(term)) ||
      (log.place && String(log.place).toLowerCase().includes(term)) ||
      (log.status && String(log.status).toLowerCase().includes(term)) ||
      (log.remarks && String(log.remarks).toLowerCase().includes(term)) ||
      (log.driver_contact && String(log.driver_contact).toLowerCase().includes(term))
  ...
  const filteredTamperingLogs = tamperingLogs.filter((log) => {
    ...
    return (
      ...
      (log.tampering_details && String(log.tampering_details).toLowerCase().includes(term)) ||
      (log.rejection_reason && String(log.rejection_reason).toLowerCase().includes(term))
    )
  })

  const adminSearchTerm = search.trim().toLowerCase()
  const filteredDirectory = clientList.filter((c) => {
    if (!adminSearchTerm) return true
    return (
      (c.company_name && c.company_name.toLowerCase().includes(adminSearchTerm)) ||
      (c.client_id_number && c.client_id_number.toLowerCase().includes(adminSearchTerm)) ||
      (c.contact_email && c.contact_email.toLowerCase().includes(adminSearchTerm))
    )
  })
  const filteredProfiles = profiles.filter((p) => {
    if (!adminSearchTerm) return true
    return (
      (p.email && p.email.toLowerCase().includes(adminSearchTerm)) ||
      (p.company_name && String(p.company_name).toLowerCase().includes(adminSearchTerm)) ||
      (p.role && p.role.toLowerCase().includes(adminSearchTerm))
    )
  })
  const filteredAuditEntries = auditLogs.filter((log) => {
    if (!adminSearchTerm) return true
    return (
      (log.action && log.action.toLowerCase().includes(adminSearchTerm)) ||
      (log.entity && log.entity.toLowerCase().includes(adminSearchTerm)) ||
      (log.performed_by && log.performed_by.toLowerCase().includes(adminSearchTerm)) ||
      (log.details && String(log.details).toLowerCase().includes(adminSearchTerm)) ||
      (log.record_id && String(log.record_id).toLowerCase().includes(adminSearchTerm))
    )
  })
    ...
    <div id="admin-app-shell" className={`${isHorizontalLayout ? 'flex flex-col' : 'flex flex-row'} min-h-screen lg:h-screen bg-linear-to-b from-slate-50 to-slate-100/60 font-sans text-slate-900 overflow-hidden relative`} dir={appSettings.orientation}>
      ...
      {isHorizontalLayout ? (
        ...
        <div className="p-4 border-t border-slate-800">
          ...
          <div className="text-center pb-2"><p className="text-[9px] font-black text-slate-500 uppercase tracking-widest leading-relaxed">© {new Date().getFullYear()} All Rights Reserved<br/>For Pawan&Team</p></div>
        </div>
          </aside>
        )
      })()}
      ...
      <header className="bg-linear-to-r from-white/95 via-indigo-50/50 to-white/95 px-4 py-4 sm:px-6 lg:px-8 border-b border-slate-300/70 shrink-0 sticky top-0 z-10 shadow-md shadow-slate-300/30 backdrop-blur">
        ...
        <div><h2 className="text-xl sm:text-2xl font-extrabold text-slate-800 tracking-tight">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap lg:items-center lg:justify-end">
            ...
            {(activeTab === 'overview' || activeTab === 'pipeline' || activeTab === 'risk' || activeTab === 'tampering' || activeTab === 'directory' || activeTab === 'clients' || activeTab === 'audit') && (
              <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-3 py-2 shadow-sm w-full sm:min-w-[220px] sm:max-w-md sm:flex-1 lg:max-w-lg">
                <Search size={16} className="text-slate-400 shrink-0" aria-hidden />
                <input
                  type="search"
                  placeholder="Search database…"
                  className="bg-transparent outline-none text-sm w-full font-semibold text-slate-700 placeholder:text-slate-400"
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  autoComplete="off"
                />
                {search ? (
                  <button type="button" onClick={() => setSearch('')} className="p-0.5 hover:bg-slate-200 rounded-md shrink-0" aria-label="Clear search">
        ...
        <div className="w-full max-w-[1600px] mx-auto flex-1 px-4 py-5 sm:px-6 lg:px-8 lg:py-8">
                        ...
                        <tr>
                          <td colSpan={5} className="px-6 py-12 text-center text-slate-500 font-medium">
                            {allLogs.length === 0
                              ...
                              : search.trim()
                                ? 'No records match your search (and date filter if applied).'
          ...
          {activeTab === 'directory' && (
            <div className={TAB_CONTAINER_STANDARD_SPACED}>
              ...
              <div className="panel-section">
                <div className="px-6 py-5 border-b border-slate-100 bg-slate-50 flex justify-between items-center"><h3 className="font-bold text-slate-800 flex items-center gap-2"><BookOpen size={18} className="text-indigo-600"/> Master Client Directory</h3><span className="bg-indigo-50 text-indigo-700 text-xs font-bold px-3 py-1 rounded-full border border-indigo-100">{search.trim() ? `${filteredDirectory.length} / ${clientList.length}` : `${clientList.length}`} Saved Clients</span></div>
                <div className="overflow-x-auto">
                  <table className="w-full text-left whitespace-nowrap">
                    <thead className="bg-white border-b border-slate-200">
                      <tr>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      ...
                      {clientList.length > 0 && filteredDirectory.length === 0 && (<tr><td colSpan={4} className="px-6 py-12 text-center text-slate-500 font-medium">No directory entries match your search.</td></tr>)}
                    </tbody>
                  </table>
                </div>
              </div>
          ...
          {activeTab === 'clients' && (
            <div className={TAB_CONTAINER_STANDARD_SPACED}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
                    ...
                    <div>
                      ...
                      <select className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl text-sm font-bold focus:ring-2 focus:ring-indigo-500 outline-none cursor-pointer" value={newUser.role} onChange={e => setNewUser({...newUser, role: e.target.value})}>
                        <option value="client">Client Dashboard Access</option><option value="admin">System Administrator</option>
                      </select>
                    </div>
                  </div>
              ...
              <div className="panel-section">
                <div className="px-6 py-5 border-b border-slate-100 bg-slate-50 flex justify-between items-center"><h3 className="font-bold text-slate-800 flex items-center gap-2"><Users size={18} className="text-indigo-600"/> Active Gateway Logins</h3><span className="bg-indigo-50 text-indigo-700 text-xs font-bold px-3 py-1 rounded-full border border-indigo-100">{search.trim() ? `${filteredProfiles.length} / ${profiles.length}` : `${profiles.length}`} Active Profiles</span></div>
                <table className="w-full text-left whitespace-nowrap">
                  <thead className="bg-white border-b border-slate-200">
                    <tr>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    ...
                    {profiles.length > 0 && filteredProfiles.length === 0 && (
                      <tr><td colSpan={4} className="px-6 py-12 text-center text-slate-500 font-medium">No profiles match your search.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
                ...
                <div className="p-6 overflow-x-auto">
                  <table className="w-full text-left whitespace-nowrap">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      ...
                      {auditLogs.length > 0 && filteredAuditEntries.length === 0 && (
                        <tr><td colSpan={5} className="px-6 py-12 text-center text-slate-500 font-medium">No audit entries match your search.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
            ...
            <div className={TAB_CONTAINER_STANDARD_SPACED_TRAILING}>
              ...
              <div className="panel-section">
                ...
                <div className="divide-y divide-slate-100">
                  {filteredTamperingLogs.length > 0 ? filteredTamperingLogs.map((incident) => (
                    ...
                    <div className="px-6 py-16 text-center">
                      <div className="mx-auto flex max-w-md flex-col items-center">
                        ...
                        <p className="text-sm font-black text-slate-800">{tamperingLogs.length > 0 ? 'No records match your search.' : 'No client responses yet.'}</p>
                        <p className="mt-1 text-sm font-medium text-slate-500">{tamperingLogs.length > 0 ? 'Try a different keyword or clear the search box.' : 'Create the first tampering incident from the form above and it will appear here with `Pending Approval` until the client responds.'}</p>
                      </div>
                    </div>
                  )}
                </div>
