...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect based on role (admin vs client portal; legacy driver role uses client)
      ...
      const looksLikeUnreachableSupabase =
        message === 'Failed to fetch' ||
        (err instanceof TypeError && message.toLowerCase().includes('fetch'))
      if (looksLikeUnreachableSupabase) {
        setError(
          'Cannot reach Supabase (network error). Set NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY in .env.local from your Supabase project settings, then restart npm run dev. If you use local Supabase, run supabase start so the API is on port 54321.'
        )
      } else {
